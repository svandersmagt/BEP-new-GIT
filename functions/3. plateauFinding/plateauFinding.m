...
if (abs(diffZmag(1)) < small && abs(diffZmag(2)) < small)
    tPlat(1).first = 1;
    nFirst = nFirst +1;
end
...
for i=2:length(diffZmag)
    if (abs(diffZmag(i)) < small && abs(diffZmag(i-1)) > small)
...
if (abs(diffZmag(end)) < small && abs(diffZmag(end-1)) < small)
...
for j = 1:nPlat
    if length(tPlat(j).first:tPlat(j).last) > nMinPointsPlat
        ...
        display(['Plateau # ' num2str(j) ' has only ' num2str(length(nMinPointsPlat)) ' points!' ])
    end
end
...
if plotThings;
    ...
    platind = find(abs(diffZmag) < small);
